...
LAYOUT_FLATPKG_DIR="${LAYOUT_DIR}/components"
...
function CreateScalarInstaller()
{
    ...
    pkgBuildCommand="/usr/bin/pkgbuild --identifier $INSTALLER_PACKAGE_ID --component-plist \"${COMPONENTSPLIST_PATH}\" --scripts \"${SCRIPTS_DIR}\" --root \"${LAYOUT_ROOT_DIR}\" \"${LAYOUT_FLATPKG_DIR}/$INSTALLER_PACKAGE_NAME.component.pkg\""
...
function CreateScalarDistribution()
{
    ...
    SCALAR_PKG_NAME="$INSTALLER_PACKAGE_NAME.component.pkg"
